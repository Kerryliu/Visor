...
Device::Device(std::string p)
    : file_path(p), name(set_name(p)),
      sensor_type_counts(set_sensor_count(p)) {
  for(unsigned int type = 0; type < sensor_types.size(); type++) {
    sensor_readings[type].resize(sensor_type_counts[type]);
  }
}

const std::vector<std::vector<std::pair<std::string, int>>> &Device::get_sensor_readings() {
  refresh_sensors();
...
const std::vector<unsigned int> Device::set_sensor_count(std::string p) {
  ...
  for(unsigned int type = 0; type < sensor_types.size(); type++) {
    ...
    for(unsigned int sensor_count = 0; sensor_count < 100; sensor_count++) {
      file.open(p + sensor_types[type] + std::to_string(sensor_count + 1) + "_input");
      if (file) {
        file.close();
      } else {
...
void Device::refresh_sensors() {
  ...
  for(unsigned int type = 0; type < sensor_types.size(); type++) {
    std::vector<std::pair<std::string, int>> sensor_type_readings(sensor_type_counts[type]);

    for(unsigned int sensor_number = 0; sensor_number < sensor_type_counts[type]; sensor_number++) {
      //Get sensor value:
      file.open(file_path + sensor_types[type] + std::to_string(sensor_number + 1) + "_input");
      ...
      file.open(file_path + sensor_types[type] + std::to_string(sensor_number + 1) + "_label");
      if(file) {
        ...
        sensor_name = temp_string + " sensor #" + std::to_string(sensor_number + 1);
      }
