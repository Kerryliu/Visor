...
int main() {
  ...
  for (auto &d : devices) {
    ...
    // std::cout << "Temperature Sensor Count: " <<
    // d.sensor_type_counts[TEMPERATURE] << std::endl;
    // std::cout << "Fan Sensor Count: " << d.sensor_type_counts[FAN] <<
    // std::endl;

    std::vector<std::vector<std::pair<std::string, int>>> readings =
        d.get_sensor_readings();
    ...
    for (auto &name_value_pair : temp_readings) {
      std::cout << name_value_pair.first << ":  "
                << name_value_pair.second / 1000 << " degrees" << std::endl;
    }

    for (auto &name_value_pair : fan_readings) {
      std::cout << name_value_pair.first << ":  " << name_value_pair.second
                << " rpm" << std::endl;
    }
    std::cout << std::endl;
  }
